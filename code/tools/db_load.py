...
def get_embeddings_file_path(file_path: str) -> str:
    ...
    embeddings_path = os.path.join(
        CONFIG.nlweb.json_with_embeddings_folder, file_name)
...
async def save_url_content(url: str) -> Tuple[str, str]:
    ...
    print(
        f"Saved URL content to temporary file: {temp_path} (type: {file_type})")
        ...
        with open(file_path, 'r', encoding='utf-8', errors='replace') as csv_file:
            ...
            try:
                ...
                if len(csv_reader.fieldnames or []) < 2:
                    print(
                        f"Warning: CSV file has fewer than 2 columns. This may not contain enough data.")
            except Exception as e:
                # If we have trouble with DictReader, try a more primitive approach
                print(
                    f"Error with DictReader: {e}. Trying with simple CSV reader...")
                csv_file.seek(0)
    ...
    try:
        ...
        for episode in podcast_episodes:
            ...
            if not url and "name" in episode:
                ...
                episode["url"] = url
                print(
                    f"Generated synthetic URL for episode: {episode['name']}")
            elif not url:
            ...
            json_data = json.dumps(
                episode, ensure_ascii=False).replace("\n", " ")
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            if has_embeddings:
                ...
                print(
                    f"File exists but doesn't contain embeddings: {file_path}")
            ...
            try:
                ...
                if len(batch_documents) >= batch_size or i == total_lines - 1:
                    if batch_documents:
                        ...
                        total_batches = (
                            total_lines + batch_size - 1) // batch_size
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            resolved_path = os.path.join(
                CONFIG.nlweb.json_data_folder, file_path)
            if not os.path.exists(resolved_path):
        ...
        if has_embeddings and not force_recompute:
            print(
                f"File already contains embeddings, switching to direct loading mode...")
        ...
        if os.path.exists(embeddings_path) and not force_recompute:
            ...
            if sys.stdin.isatty():
                ...
                use_existing = True
                print(
                    f"Using existing file with embeddings at {embeddings_path}")
        ...
        # 打印环境变量及其值
        print(
            f"The value of OPENAI_EMBEDDING_ENDPOINT is: {embedding_endpoint}")
        ...
        if file_type == 'csv':
            ...
            for line in lines:
                try:
                    ...
                    # Prepare documents
                    documents, _ = prepare_documents_from_json(
                        url, json_data, site)
                ...
                for i in range(0, len(all_documents), batch_size):
                    ...
                    if batch_docs and batch_texts:
                        try:
                            ...
                            batch_idx = i // batch_size
                            total_batches = (
                                len(all_documents) + batch_size - 1) // batch_size
                    ...
                    if file_type == 'rss' or file_type == 'xml':
                        ...
                        if docs:
                            ...
                            for j in range(0, len(docs), batch_size):
                                ...
                                await client.upload_documents(docs_with_embeddings)
                                print(
                                    f"Successfully uploaded batch {batch_idx+1}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load data into vector database")
